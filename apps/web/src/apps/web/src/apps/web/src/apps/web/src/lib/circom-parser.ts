...
export interface ParsedCircuit {
  ...
  nPrivate: number;
  nOutputs: number;
  prime: string; // hex-строка простого числа
}
...
export async function parseCircomFile(file: File): Promise<ParsedCircuit> {
  ...
  for (const sec of sections) {
    ...
    if (sec.type === 1) {
      ...
      // Читаем простое число (prime) как массив байт в little-endian и преобразуем в hex
      const primeBytes = sectionData.slice(sectionOffset, sectionOffset + fieldSize);
      // Переворачиваем байты для big-endian hex (как принято в circom)
