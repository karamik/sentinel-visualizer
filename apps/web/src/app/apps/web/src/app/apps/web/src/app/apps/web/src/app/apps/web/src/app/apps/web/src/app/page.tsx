...
import { useSubscription } from '@/contexts/SubscriptionContext' // <-- ИМПОРТ

// Встроенные примеры (без изменений)
...
export default function Home() {
  const { plan, features } = useSubscription() // <-- ПОЛУЧАЕМ ПЛАН И ФИЧИ
  ...
  // Проверка, превышен ли лимит узлов для бесплатного плана
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {allNodes.length > 0 && !nodeLimitExceeded && (
            <div className="flex justify-end gap-2 mb-2">
              ...
              {features.can3D && (  // Показываем кнопку 3D только если есть доступ
                <button
