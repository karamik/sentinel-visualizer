...
import { CircuitGraph3D } from '@/components/CircuitGraph3D' // <-- ДОБАВЛЕНО
...
export default function Home() {
  ...
  const [show3D, setShow3D] = useState(false) // <-- ДОБАВЛЕНО
  ...
  // ВРЕМЕННО: показываем 3D даже без схемы для теста
  if (show3D) {
    return (
      <main className="min-h-screen p-4">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold text-primary">3D Test</h1>
          <button
